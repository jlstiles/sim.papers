...
get.info = function(n, d, truth) {
  ...
  PG_n  = truncate(plogis(dfs[[1]][[1]] %*% coef_G), .05)
  A = rbinom(n, 1, PG_n)
  mean(A)
  ...
  if (truth) {
    ...
    PG_true  = truncate(plogis(dfs[[1]][[2]] %*% coef_G), .05)
    ...
    while (BV0 <= .025 & jj < 6) {
      ...
      PQ_true1  = truncate(plogis(df1_true %*% coef_Q), .05)
      PQ_true0 = truncate(plogis(dfs[[2]][[2]] %*% coef_Q[1:ncol(dfs[[2]][[2]])]), .05)
  ...
  PQ_n  = truncate(plogis(dfQn %*% coef_Q), .05)
