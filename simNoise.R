...
biasQ = function(A,W1,W2,W3,W4,n,rate)  n^-rate*3*(-.2+1.5*A+.2*W1+1*W2-A*W3+ 1*W4)
# hist(with(truth,biasQ(A,W1,W2,W3,W4,n=n,rate=rate)))
sdQ = function(A,W1,W2,W3,W4,n,rate) (n^-rate)*1.5*(abs(3.5+.5*W1+.15*W2+.33*W3*W4-W4))
...
cov1step0 = lapply(1:40, FUN = function(x) getRes(L[[x]],1000)[[3]][2])
plot(c(seq(250,10000,250)[1:40]),cov1step0)

# save(L,var0, ATE0, Q0, g0, biasQ, sdQ, noise.1, noise.0, noise,
#      file="~/Dropbox/Jonathan/Simulations/simNoiseBest1to40.RData")
